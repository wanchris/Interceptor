...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    //register ax holds the syscall number of the syscall being intercepted
    //use it to find monitor status for the syscall in table  
    if (table[reg.ax].monitored == 1){
        if (check_pid_monitored(reg.ax, current->pid)){
            log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, 
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    if (syscall < 0 || syscall > (NR_syscalls - 1) || syscall == 0){
	  return (-EINVAL);
    } 

    if (pid < 0 ||  (pid != 0 && pid_task(find_vpid(pid), 
                    PIDTYPE_PID) == NULL)) {
        return (-EINVAL);
    }

    //current_uid returns the user id, which is 0 for root
    ...
    if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING){
        if (current_uid() != 0 && (pid == 0 || (check_pids_same_owner(pid, 
                            current->pid) != 0))) {
            return (-EPERM);
        }
    }
    ...
    if (cmd == REQUEST_SYSCALL_INTERCEPT){
        ...
        //syscall is already intercepted 
        ...
        //store the original syscall and replace it with our interceptor
        ...
        //syscall isn't being intercepted
        if (table[syscall].intercepted != 1){
            spin_unlock(&my_table_lock); 
        ...
        if (table[syscall].monitored == 2 || check_pid_monitored(syscall, 
                    pid) == 1){
    	    spin_unlock(&my_table_lock);
	    ...
	    if (pid == 0){ 
			//if the pid is 0, everything must be monitored 
			//set table[syscall].monitored to 2 - the interceptor will 
            //handle this 
		    ...
		    if (add_pid_sysc(pid, syscall) != 0) {
		        spin_unlock(&my_table_lock);
 				//system has ran out of memory 
			    return (-ENOMEM); 
		    }
			//some values are now being monitored for syscall
		...
		set_addr_rw((unsigned long) sys_call_table);
		//if the pid is 0, remove all pids
		if (pid == 0) { 
...
static int init_function(void) {
	int x; //create a variable for counter 

	spin_lock(&my_table_lock);

	//initialize all table values to 0 for bookkeeping 
	for (x = 0; x < NR_syscalls; x++){
		table[x].f = NULL;
...
static void exit_function(void)
{
    ...
    //all syscalls in table, NR_syscalls, check if intercepted, if so 
    //deintercept and remove all pids for that syscall
    for (x = 0; x <= NR_syscalls; x++){
        spin_lock(&my_table_lock);
        
        if (table[x].intercepted){
            table[x].intercepted = 0;
            
            if (table[x].monitored){
	            destroy_list(x);
            } 
        }   
